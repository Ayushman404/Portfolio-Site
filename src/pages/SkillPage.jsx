...
const tools = ['GSAP', 'Redux', 'Axios', 'Node.js', 'Git & GitHub', 'VS Code', 'Postman'];
const softSkills = ['Fast Learner', 'Team Player', 'Creative Thinker', 'Curious by Nature'];
    ...
    <section id="skills" className="w-full min-h-screen text-center flex flex-col gap-4 px-4 py-6 bg-gradient-to-br from-secondary-bg to-violet-400/40 overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between gap-6 w-full flex-grow">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-4 gap-4 w-full lg:w-[65%] justify-items-center px-2 lg:px-8">
          {skills.map((skill, i) => (
            <div key={i} className="skillCircle relative w-20 h-20 sm:w-24 sm:h-24 lg:w-28 lg:h-28 flex items-center justify-center">
        ...
        <div className="w-full lg:w-[35%] flex flex-col items-center justify-evenly text-center px-2 gap-4 h-full">
          <div className="w-full bg-white rounded-xl shadow-lg p-4">
            <h2 className="text-2xl font-bold text-primary-text mb-2 text-shadow-secondary-text text-shadow-3xs">Currently Learning</h2>
            <ul className="list-disc list-inside text-sm text-left space-y-1">
              {learning.map((item, i) => (
                <li key={i} className="text-gray-700 font-medium tracking-wide pl-1">{item}</li>
              ))}
            </ul>
          </div>

          <div className="w-full bg-white rounded-xl shadow-lg p-4">
            <h2 className="text-xl font-semibold text-primary-text mb-2 text-shadow-secondary-text text-shadow-3xs">Libraries & Tools</h2>
            <div className="flex flex-wrap gap-2 justify-center">
              {tools.map((tool, i) => (
                <span key={i} className="px-3 py-1 bg-purple-200 text-purple-800 rounded-full text-xs font-medium shadow-sm">{tool}</span>
              ))}
            </div>
          </div>

          <div className="w-full bg-white rounded-xl shadow-lg p-4">
            <h2 className="text-xl font-semibold text-primary-text mb-2 text-shadow-secondary-text text-shadow-3xs">Strengths</h2>
            <div className="flex flex-wrap gap-2 justify-center">
              {softSkills.map((skill, i) => (
                <span key={i} className="px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-medium shadow-sm">{skill}</span>
              ))}
            </div>
          </div>
        </div>
